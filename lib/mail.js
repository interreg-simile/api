...
async function initTransporter(logger) {
  ...
  return new Promise((resolve, reject) => {
    transporter.verify(error => {
      if (error) {
        logger.error({ err: error }, '[setup] Error loading mail transporter')
        reject(error)
      } else {
...
module.exports = {
  ...
  getConfirmationEmailTemplates: () => confirmationEmailTemplates,
  getResetPasswordTemplates: () => resetPasswordTemplates,
}
