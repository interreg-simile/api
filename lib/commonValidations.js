...
const queryIncludeDeletedOnlyIfAdmin = query('includeDeleted')
  .optional()
  .isBoolean().withMessage('Must be a boolean')
  .custom((value, { req }) => {
    if (value !== 'false' && !req.isAdmin) {
      throw new Error('Can be set to true only by an admin user')
    }
...
const querySort = query('sort')
  .optional()
  .custom((value, { req }) => validateSortQuery(value, req.config))

function validateSortQuery(value, { sort: sortConfig }) {
  if (!sortConfig) {
  ...
  for (const sort of value.split(',')) {
    ...
    if (!sortConfig.includes(sortElements[0])) {
...
module.exports = {
  paramId,
  queryIncludePast,
  queryIncludeDeletedOnlyIfAdmin,
  querySort,
  customLanguageKeys,
}
